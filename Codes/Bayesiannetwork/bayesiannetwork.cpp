...
bayesiannetwork::bayesiannetwork(int a)
{
	a=a+1;
	...
	for(int f2=0 ; f2< attributes ; f2++)//f2 0-6
	{
		for(int f3=(f2*numclass[attributes]) ; f3 < ( (f2+1)*numclass[attributes] ) ; f3++)//0-7 8-15 6*8 7*8 55
		{
	...
	{
		for(int ff1=(ff*numclass[attributes]) ; ff1 < ( (ff+1)*numclass[attributes] ) ; ff1++)//0-7 8-15
		{
...
//        for(int bb=0;bb <= attributes;bb++)
//        {
//            std::cout<<oneLine[bb]<<std::endl;
//        }
		for (int h=0 ; h< attributes ; h++)
        {
            ccc[h*numclass[attributes]+oneLine[attributes]-1][oneLine[h]-1 ]++;
            //std::cout<<h*numclass[attributes]+oneLine[attributes]-1<<std::endl;
        }
...
{
	std::ifstream testInputFile("adult_test.txt",std::ios::in);
